...
/**
 * Controller for watercraft-like vehicles (e.g boats).
 * <br>
 * @author AdrianSR / 28/12/2023 / 2:55 p. m.
 */
public class WatercraftVehicleController extends VehicleController {
	 ...
	 */
	public WatercraftVehicleController ( @NotNull Vehicle vehicle ,
	...
	protected void loadProperties ( ) {
		maxPositiveAcceleration = properties.getDoubleProperty (
				"max-positive-acceleration" , 0.12 );
		maxNegativeAcceleration = properties.getDoubleProperty (
				"max-negative-acceleration" , 0.22 );
		...
		negativeAccelerationOnWater = properties.getDoubleProperty (
				"negative-acceleration-on-water" , 0.025 );
		negativeAccelerationOnLava  = properties.getDoubleProperty (
				"negative-acceleration-on-lava" , 0.025 );
		...
		turningAngleMax = properties.getIntegerProperty ( "turning-angle-max" , 3 );
	...
	public void tick ( ) {
		...
		// applying speed
		if ( currentSpeed != 0.0D && ( isOnLiquid (
	...
	protected boolean isOnLiquid ( @NotNull Map < EnumSurface, Double > surface ) {
		return surface.containsKey ( EnumSurface.WATER )
				|| surface.containsKey ( EnumSurface.LAVA );
	}
